...
class AWSIoTMQTTPubSub:
    ...
    def __init__(self, endpoint: str, port: int, client_id: str, certificate: str, private_key: str, root_ca: str, topic: str, logger=None, debug=False):
        self.logger = None
        if logger:
    ...
    def self_subscription(self):
        ...
        try:
            ...
            if self.logger:
                self.logger.error(
                    f"Failed to subscribe to topic {self.topic}\n{type(e).__name__}: {e}")
            else:
                print(
                    f"Failed to subscribe to topic {self.topic}\n{type(e).__name__}: {e}")
