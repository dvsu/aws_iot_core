...
from aws.iotcore.nameshadowupdater import AWSMQTT
        ...
        while True:
            try:
                ...
                for i in range(1, 4):
                    ...
                    with open(f'dummy_device_data_{i}.json', 'r') as file:
                        data = json.load(file)

                    # data["logtime"] = datetime.now().timestamp()
                    # data["ttl"] = data["logtime"] + config.TTL
                    self.aws_mqtt.publish_message(
                        message=data, name_shadow=f'{config.TOPIC_PREFIX}{i}/update')
